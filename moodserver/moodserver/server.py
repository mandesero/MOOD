...
class Game:
    ...
    @staticmethod
    def add_monster(monster: Optional[Monster]) -> Tuple[MonsterMeta, bool, str]:
        """
        Adds monster to the game field

        :param monster: (Monster) monster instance
        :return: (Tuple[MonsterMeta, bool]) monster name, message and coordinates, replacing flag and name of old monster
        """
        ...
        if Game.field[i][j]:
            replace = True
            old_name = Game.field[i][j].name
            if old_name != monster.name:
                Monster.pop(old_name)

        Game.field[i][j] = monster

        return monster.meta(), replace, old_name
        ...
        match msg:
            ...
            case ["addmon", *args]:
                try:
                    ...
                    *tmp, replace, old_name = Game.add_monster(new_monster)
                    ...
                    for name, clt in Client.client_list.items():
                        if clt is client:
                            ...
                            if replace:
                                ans += _("\n{} has been replaced.").format(old_name)
